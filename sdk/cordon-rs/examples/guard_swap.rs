//! Minimal example of putting a candidate transaction through the Cordon
//! firewall with the async client.
//!
//! Run against a local validator with the program deployed:
//!
//! ```bash
//! cargo run -p cordon-rs --features client --example guard_swap
//! ```
//!
//! Set `CORDON_RPC` (defaults to localhost) and `CORDON_PAYER` (path to a
//! keypair JSON, defaults to the standard Solana CLI keypair).

use std::sync::Arc;

use cordon_rs::client::{
    read_keypair_file, AccountMeta, Candidate, CordonClient, GuardOutcome, Instruction, Keypair,
    Pubkey, Signer,
};
use cordon_rs::find_agent_pda;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let rpc = std::env::var("CORDON_RPC").unwrap_or_else(|_| "http://127.0.0.1:8899".to_string());
    let payer_path = std::env::var("CORDON_PAYER").unwrap_or_else(|_| {
        let home = std::env::var("HOME").unwrap_or_default();
        format!("{home}/.config/solana/id.json")
    });

    let payer = Arc::new(read_keypair_file(&payer_path).map_err(|e| e.to_string())?);
    let client = CordonClient::new_with_rpc(rpc, payer)?;
