//! Async on-chain client for the Cardon program.
//!
...
use cardon::state::{Agent, PendingStatus, PendingTx, Policy};
use cardon::ID as CARDON_PROGRAM_ID;
use thiserror::Error;
...
type Result<T> = std::result::Result<T, CardonClientError>;
...
/// Async client over the Cardon Anchor program. The `payer` it is constructed
...
impl CardonClient {
    ...
    pub fn new(cluster: Cluster, payer: Arc<Keypair>, commitment: CommitmentConfig) -> Result<Self> {
        ...
        let program = client.program(CARDON_PROGRAM_ID)?;
    ...
    pub async fn simulate(&self, instructions: &[Instruction]) -> Result<RpcSimulateTransactionResult> {
        ...
        let response = rpc
            ...
            .await
            .map_err(|e| CardonClientError::Rpc(e.to_string()))?;
        let value = response.value;
        if let Some(err) = value.err {
            return Err(CardonClientError::SimulationFailed {
    ...
    pub async fn register_agent(
        ...
        let signature = self
            .program
            .request()
            .accounts(cardon::accounts::RegisterAgent {
            ...
            .args(cardon::instruction::RegisterAgent {
                max_lamports_per_tx,
    ...
    pub async fn set_policy(
        ...
        Ok(self
            .program
            .request()
            .accounts(cardon::accounts::SetPolicy {
                authority: authority.pubkey(),
                agent,
                policy,
            })
            .args(cardon::instruction::SetPolicy {
                max_lamports_per_tx,
                hitl_threshold_lamports,
                daily_volume_ceiling,
                allowed_programs,
    ...
    pub async fn set_enabled(
        ...
        Ok(self
            .program
            .request()
            .accounts(cardon::accounts::SetEnabled {
                authority: authority.pubkey(),
                agent,
            })
            .args(cardon::instruction::SetEnabled { enabled })
            .signer(authority.clone())
            .send()
            .await?)
    }
    ...
    pub async fn submit_auto(
        ...
        Ok(self
            .program
            .request()
            .accounts(cardon::accounts::SubmitAuto {
                agent_signer: agent_signer.pubkey(),
                agent,
                policy,
            })
            .args(cardon::instruction::SubmitAuto {
                tx_hash,
                lamports,
                target_program,
            })
    ...
    pub async fn submit_for_review(
        ...
        let signature = self
            .program
            .request()
            .accounts(cardon::accounts::SubmitForReview {
                payer: self.payer.pubkey(),
                agent_signer: agent_signer.pubkey(),
                agent,
                policy,
            ...
            .args(cardon::instruction::SubmitForReview {
                tx_hash,
                lamports,
                target_program,
            })
    ...
    pub async fn approve_tx(
        ...
        Ok(self
            .program
            .request()
            .accounts(cardon::accounts::ApproveTx {
                authority: authority.pubkey(),
                rent_receiver,
                agent,
                policy,
            ...
            .args(cardon::instruction::ApproveTx)
            .signer(authority.clone())
            .send()
            .await?)
    }
    ...
    pub async fn reject_tx(
        ...
        Ok(self
            .program
            .request()
            .accounts(cardon::accounts::RejectTx {
                authority: authority.pubkey(),
                rent_receiver,
                agent,
                pending,
            })
            .args(cardon::instruction::RejectTx { reason_code })
            .signer(authority.clone())
            .send()
            .await?)
    }
