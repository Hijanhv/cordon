...
export interface CordonPluginConfig {
  client: CordonClient;
...
/**
 * A Solana Agent Kit plugin that exposes the Cordon firewall to the agent's
 * LLM as callable actions, and surfaces the policy/queue helpers on
 * `agent.methods`. Pair it with {@link CordonWallet} for actual enforcement.
 *
 * ```ts
 * agentKit.use(createCordonPlugin({ client, agent }));
 * ```
 */
export function createCordonPlugin(config: CordonPluginConfig): Plugin {
  const getPolicy: Action = {
    name: "CORDON_GET_POLICY",
    similes: [
    ...
    description:
      "Fetch the on-chain Cordon policy (per-tx cap, HITL threshold, daily volume ceiling, program allowlist) for this agent.",
    examples: [
      [
        {
          input: {},
  ...
  const listPending: Action = {
    ...
    description:
      "List transactions the Cordon firewall has queued for human approval for this agent.",
    examples: [
      [
        {
          input: {},
