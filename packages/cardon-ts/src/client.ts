...
import { CARDON_PROGRAM_ID, findPendingPda, findPolicyPda } from "./pda";
...
/**
 * TypeScript client over the Cardon Anchor program — the off-chain spine of the
 * firewall. Mirrors `cardon_rs::client::CardonClient`.
 */
export class CardonClient {
  readonly program: Program<Cardon>;
  readonly provider: AnchorProvider;

  constructor(provider: AnchorProvider) {
    this.provider = provider;
    this.program = new Program(cardonIdl as Cardon, provider);
  }
  ...
  static fromConnection(
    ...
    return new CardonClient(new AnchorProvider(connection, wallet, opts));
  }
