...
TypeScript binding for the [Cordon](../../README.md) on-chain transaction firewall. Mirrors the Rust SDK (`cordon-rs`): PDA helpers, a pure client-side `preview()` of the on-chain policy, a SHA-256 `txHash` helper, transaction intent extraction, and a `CordonClient` over the Anchor program with a `guard()` firewall path.
...
- `CORDON_PROGRAM_ID`, `findAgentPda`, `findPolicyPda`, `findPendingPda`
...
- `CordonClient` — full async client (fetch, instruction builders, `simulate`, `guard`)
