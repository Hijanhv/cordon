# Capstone Project Proposal: Cordon
...
3. **Solana protocols with agentic flows.** Drift, Jupiter, Kamino, Marginfi already integrate agentic execution. The protocols want to keep agent behavior inside their risk envelope without writing per-agent firewalls themselves. Cordon is the layer they recommend to integrating agent teams.
...
|---|---|---|
| **Squads Multisig** | Human-driven approval workflows | Built for low-throughput human multisig, not for the volume an agent generates. Cordon's HITL queue is a natural Squads integration, not a Squads replacement. |
...
- **Squads.** The AI didn't surface it at all in its initial competitor list. This is the most important manual add: Squads is the canonical Solana multisig primitive and the obvious *integration* target for Cordon's HITL queue. Treating it as a competitor was the wrong frame; integrating with it from week 1 is the right one.
- **OpenZeppelin Defender, weighted properly.** The AI named Defender but treated it as roughly equivalent to Sentinel. It's not — Defender is a much bigger company with an active non-EVM expansion roadmap. If they ship Solana support before Cordon mainnet, the only moat left is SVM-native policy primitives (rent-aware caps, CPI-depth limits, compute-budget gating). That's a real risk to plan around, not a feature parity issue.
- **SendAI absorbing the category.** The AI missed that SendAI could ship a built-in policy layer in the Agent Kit itself, which would compress Cordon's wedge to nothing for SendAI-only users. The counter is to ship Cordon as a SendAI plugin from day one (their `packages/adapter-*` interface is already documented) so that the *official* way to add policy hooks is via Cordon, not against it.
- **Hackathon winners as competitors vs. customers.** Every AI-track Cypherpunk and Breakout winner I looked at is a Cordon integration candidate, not a competitor. The AI didn't make this distinction — it lumped them in as competitive overlap when they're actually the beachhead customer list.

## 4. Founder-Market Fit

I'm in the Turbin3 Builders cohort and Cordon is my capstone build. My fit for this specific project comes from three things that are verifiable without overclaiming credentials I don't have:

**1. The timing match is real, not retrofitted.** Turbin3's CEO Nate Hughes published the "AI verification and security on Solana" thesis post in May 2026. The next Turbin3 institute module is literally named *Solana Model and Agent Verification.* The cohort itself has pre-committed to caring about this category — my judging bench, my mentors, and my evaluation rubric are aligned with the problem space before I even started. I'm not pitching uphill.

**2. The cohort is the distribution channel.** Every Solana agent team I'd want to integrate with is one warm intro away inside the Turbin3 alumni graph. Alumni are placed at Anagram, Paladin, Metaplex, Drift, Compute Labs, and 200+ partner protocols. The motion for getting the first 5 pilot teams is "ask in the cohort Slack and the alumni channel," not cold outbound. That's a real distribution advantage I get specifically because I'm in this cohort, not despite it.

**3. Scope match.** A 6-week Anchor program plus a Rust SDK plus a Next.js HITL dashboard is what the cohort grades on. It's exactly the wrong scope to handwave as a side project and exactly the right scope to ship under cohort accountability. I have the mentor access and the deadline structure to actually finish it.

**Honest weakness.** I don't have a published security-research history. B2D infra sales to sophisticated agent operators don't close on marketing or a pitch deck; they close on a real incident, a credible response, and a track record over months. The counter-motion is the only one available to a single capstone builder: open-source the SDK from day one, free hosted tier, get hackathon teams as reference integrations through the cohort and Colosseum networks, then earn enterprise trust after Cordon has caught its first real attack in production. Enterprise sales come after the credibility, not as the launch motion.
...
- **Squads Multisig.** Most important miss. Squads is the canonical Solana multisig and is the obvious integration target for Cordon's HITL queue, not a competitor. The AI didn't surface it at all.
...
- **SendAI internal policy hooks.** The AI didn't realize SendAI's own kit has a basic `actions` interface that includes light allow/deny logic. This is the absorb-the-category risk, and the response is to ship Cordon as a SendAI plugin from day one rather than against it.
...
> "Critique this project's value prop, target market, and competitive analysis (Cordon, on-chain firewall for Solana AI agents). Why might this not be a true blue ocean? What types of competitors might have been overlooked?"
...
3. **Squads cannibalization.** If Squads ships an agent-wallet multisig primitive, Cordon's HITL value collapses to "Squads + Helius simulation + a dashboard."
...
*Reason:* the most dangerous "competitor" is not another firewall — it's the protocol-layer absorb move from Squads, Helius, or SendAI. The response is to ship integrations *with* the most likely absorbers (SendAI plugin, Squads-backed HITL) from week 1, so that when the absorb question comes up the answer is "Cordon is already how SendAI does this."
...
**My response.** Valid. The original draft of the FMF leaned on "experienced Web3 builder" framing that wasn't honest about my actual stage. I rewrote it to stand on three verifiable structural points (cohort timing match, cohort network access, cohort scope match) and named the credibility weakness explicitly rather than papering over it. The honest framing is more credible to a security-infra-aware reader than the overclaim version, because anyone evaluating this seriously will see through the overclaim immediately.
