...
Two off-chain services that sit beside the Cardon program:
...
Env vars: `CARDON_RPC`, `AUDIT_DIR`, `WEBHOOK_PORT`, `POLL_MS`, `HELIUS_AUTH` (optional Authorization token the webhook requires).

Point a Helius webhook at `http://<host>:<port>/helius` (account address: the Cardon program ID) to feed broadcast confirmations.

## Use as a library

```ts
import { CardonIndexer, createWebhookServer, JsonlAuditStore } from "cardon-audit-mirror";

const store = new JsonlAuditStore("./audit-data");
const indexer = new CardonIndexer(connection, store);
